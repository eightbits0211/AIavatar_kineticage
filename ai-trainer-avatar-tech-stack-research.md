...
> **⚠️ Historical document.** This captures the pre-build technology evaluation. Several choices changed during implementation — most notably the **AI provider is Google Gemini, not Anthropic Claude** (text: Gemini 2.5 Flash; voice: Gemini Live `gemini-3.1-flash-live-preview`). References to "Claude" below reflect the original evaluation, not the shipped system. For the current architecture see [system-design.md](./system-design.md) and [docs/backend-handover.md](./docs/backend-handover.md).
